...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const {
      brand, model, year, type, lengthFt, sleeps, slides,
      weightLbs, msrp, floorplanType, description, imageUrl,
      submittedBy, featureKeys,
    } = req.body
    ...
    const rv = await prisma.rV.create({
      data: {
        brand, model, year: Number(year), type,
        lengthFt: Number(lengthFt), sleeps: Number(sleeps),
        slides: Number(slides) || 0,
        weightLbs: weightLbs ? Number(weightLbs) : null,
        msrp: msrp ? Number(msrp) : null,
        floorplanType, description, imageUrl, submittedBy,
        status: 'PENDING',
        features: { create: featureConnections },
      },
